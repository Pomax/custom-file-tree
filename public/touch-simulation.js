// Adapted from https://raw.githubusercontent.com/hammerjs/simulator/60f43a021a92cadfcb48126f8aba819020f6bcb7/index.js

if (globalThis.Simulator) {
  throw new Error(`not loading Simulator, globalThis.Simulator already exists`);
}
...
(function () {
  ...
  Simulator.events = {
    ...
    pointer: {
      ...
      trigger: function (touches, element, type) {
        touches.forEach(({ x, y }, i) => {
          var eventType = TYPE_MAP[type];
  ...
  function triggerGesture(elements, startTouches, options, done) {
    ...
    function gestureLoop() {
      ...
      if (options.scale !== 1) {
        radius =
          options.radius -
          options.radius * (1 - options.scale) * ((1 / loops) * loop);
      }
  ...
  Simulator.gestures = {
    drag: function (from, to, options, done) {
      const { left, width, top } = from.getBoundingClientRect();
      options = merge(options, {
        pos: [left + width / 2, top + 1],
        duration: 50,
        touches: 1,
      });

      var touches = getTouches(options.pos, from, 1);
      trigger(touches, from, "start");
      ...
      function drag(i = 0) {
        if (i === steps - 1) {
          trigger(touches, to, "end");
          return done && setTimeout(done, 25);
        }
        touch.x += dx;
        touch.y += dy;
        touch.target = document;
        trigger(touches, to, "move");
  ...
  // initial
  if (globalThis.PointerEvent || globalThis.MSPointerEvent) {
